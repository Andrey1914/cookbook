...
import Filters from "@/components/Recipes/FilterGroup/Filters";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

export default function RecipeGrid({ filters }) {
  ...
  const fetchRecipes = useCallback(async (start = 1, end = 8) => {
    try {
      let url = `/api/recipe?start=${start}&end=${end}`;
      const res = await fetch(url);
  ...
  const fetchFilteredRecipes = useCallback(
    async (checkedItems, start = 0, end = 7) => {
      const options = {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
